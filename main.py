...
def main(args):
    ...
    for name_time_seq_t in name_time_table:
        ...
        time_seq = []
        station_occurrence_tracker = {}  # 各列車ごとにリセット
        for name in all_name_seq:
            if name in check_name_seq:
                # この駅の使用予定の出現番号を取得
                if name not in station_occurrence_tracker:
                    station_occurrence_tracker[name] = 0
                
                station_occurrence_tracker[name] += 1
                target_occurrence = station_occurrence_tracker[name]
                total_occurrences = check_name_seq.count(name)
                
                # target_occurrence番目の出現を探す（超過した場合は最後の出現）
                count = 0
                point = -1
